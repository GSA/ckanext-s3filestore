...
class S3FileStorePlugin(plugins.SingletonPlugin):
    ...
    def configure(self, config):
        ...
        if not config.get('ckanext.s3filestore.aws_use_iam_role'):
            config_options = config_options + ('ckanext.s3filestore.aws_access_key_id')
            config_options = config_options + ('ckanext.s3filestore.aws_secret_access_key')
            config_options = config_options + ('ckanext.s3filestore.region_name')
            config_options = config_options + ('ckanext.s3filestore.signature_version')
            config_options = config_options + ('ckanext.s3filestore.host_name')
